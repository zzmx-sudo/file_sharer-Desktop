...
### 特征
- 简洁显示下载列表, 文件夹合一
- 路径操作采用`Path`替换`os.path`

### 优化
- 优化后端服务DEBUG日志显示, 日志正确告知流程进度
- 优化FTP目标文件cwd路径计算, 过滤不必要的计算
- 优化删除下载记录后的环境清除
- 优化文件类复用, 减少冗余逻辑, 前端复用`BrowseFileDictModel`, 后端复用`FileModel`
- 优化首屏显示效率
...
### [0.4.3] - 20251115
### 特征
- 客户端根据上传/下载速率动态加载分片大小, 保证稳定性同时提高速率
